...
class UserSession:
    ...
    def login(self):
        ...
        try:
            logger.info(f"Выполняется авторизация для пользователя {self.user_id}...")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        logger.warning("Команда /start вызвана без пользователя")
        if update.message:
    ...
    if user_id not in user_sessions:
        ...
        await update.message.reply_text(
            ...
            "Доступные команды:\n"
            "/login - принудительная авторизация\n"
...
async def login_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if not user:
        logger.warning("Команда /login вызвана без пользователя")
        if update.message:
            await update.message.reply_text("Не удалось определить пользователя")
        return
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if not user:
        logger.warning("Команда /status вызвана без пользователя")
        if update.message:
            await update.message.reply_text("Не удалось определить пользователя")
        return
    ...
    if user_id in user_sessions:
        ...
        last_auth = session.last_auth.strftime("%H:%M:%S %d.%m.%Y") if session.last_auth else "никогда"
...
async def cancel_direction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if not user:
        logger.warning("Сообщение без пользователя")
        await update.message.reply_text("Не удалось определить пользователя")
        return
...
def main():
    ...
    try:
        # Создаем приложение с более детальными настройками
